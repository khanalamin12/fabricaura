...
function productCard(p) {
  ...
  return `
    <div class="product-card" data-section="${p.section}" onclick="openDetail(${p.id})">
...
renderSections();
const initBanner=document.getElementById("brandBanner"); if(initBanner) initBanner.style.display="block";

// ══════════════════════════════════════════════════════════════════
// SEARCH by khan Al-amin
// ══════════════════════════════════════════════════════════════════
function openSearch() {
  searchOpen = !searchOpen;
  document.getElementById('searchBar').style.display = searchOpen ? 'flex' : 'none';
  if (searchOpen) {
    const inp = document.getElementById('searchInput');
    inp.focus();
    // Dismiss keyboard on Enter (mobile)
...
function filterProducts(q) {
  ...
  renderSections(scored.map(x => x.p));


  const existing = document.getElementById('searchHint');
  if (existing) existing.remove();
  if (!hasExact && scored.length > 0) {
    const hint = document.createElement('p');
    hint.id = 'searchHint';
    hint.style.cssText = 'grid-column:1/-1;font-size:.78rem;color:var(--gray);padding:0 0 10px;display:flex;align-items:center;gap:6px;';
    hint.innerHTML = `<i class="fas fa-wand-magic-sparkles" style="color:var(--crimson)"></i> Showing closest results for "<strong>${v}</strong>"`;
    if (grid && grid.firstChild) grid.insertBefore(hint, grid.firstChild);
  }
}
    ...
    el.innerHTML = wishlist.map(id => {
      ...
      return `<div class="wish-item">
        ...
        <div class="wish-item-actions">
          ...
          <button class="wish-remove-btn" onclick="toggleWish(event,${p.id});openWishlist()">
...
function openDetail(id) {
  ...
  document.getElementById('detailContent').innerHTML = `
    <div class="detail-carousel">${carouselHtml(p, 'det')}</div>
  ...
  document.getElementById('detailStickyBtn').innerHTML = `
    <button class="order-now-btn" onclick="closeAllDrawers();setTimeout(()=>openOrderForm(${p.id}),200)">
      <i class="fas fa-bag-shopping"></i> Order This Item — ৳${p.price.toLocaleString()}
    </button>`;
...
function closeAllDrawers() {
  ['detailDrawer', 'orderDrawer', 'wishlistDrawer'].forEach(d =>
...
window.addEventListener('popstate', function(e) {
  const anyOpen = ['detailDrawer', 'orderDrawer', 'wishlistDrawer']
    .some(d => document.getElementById(d).classList.contains('open'));
  if (anyOpen) {
    closeAllDrawers();
  } else {
    // No drawer open — scroll to home instead of exiting
...
function addToRecentlyViewed(id) {
  ...
  renderRvTray();
  // Show FAB button
  const fab = document.getElementById('rvTab');
...
document.addEventListener('click', function(e) {
  const tray = document.getElementById('rvTray');
  const tab  = document.getElementById('rvTab');
  if (tray && tray.classList.contains('open') &&
      !tray.contains(e.target) && !tab.contains(e.target)) {
